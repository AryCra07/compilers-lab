...
{
public:
    explicit parser(const string &_filename) // Parser constructor and init variables
    {
        filename = _filename;
        file.open(filename, ios::in);
        memset(wordCount, 0, sizeof(int) * 6);
        token[0] = 0;
        loc.filename = _filename;
        loc.line = 0;
        loc.col = 0;
    };
...
#define LEXPARSER_PARSER_H

#endif // LEXPARSER_PARSER_H
